...
static int expr()
{
	...
	default:
		ERROR("expr error Symbol %c unknown\n", token);
		exit(EXIT_FAILURE);
	}
}
...
{	
	int reg;
	int tokena = token;
	if ((is_identifier(token))||(is_digit(token))) {
	next_token();
	}
	if (token != '='){
		ERROR("Assign error not = Symbol %c unknown\n", token);
		exit(EXIT_FAILURE);
	}
	next_token();
	reg = expr();
	CodeGen(STOREAI, reg, 0, (tokena-'a')*4);
}
static void print()
{
	int tokena = token;
	if((is_identifier(token))||(is_digit(token))){
		next_token();
		CodeGen(OUTPUTAI,0, (tokena-'a')*4, EMPTY_FIELD)	;
		}
	else{
		ERROR("Print error not = Symbole %c unknown\n", token);
		exit(EXIT_FAILURE);
	}
}
...
static void stmt()
{
	if (is_identifier(token)){
		...
		ERROR("stmt error Symbol %c unknown\n", token);
		exit(EXIT_FAILURE);
	}
}
static void morestmts()
...
static void stmtlist()
{	
	if (is_identifier(token) || is_digit(token) || token == '!'){
		stmt();
		morestmts();
	}
	else{
	ERROR ("stmtlist error Symbol %c unknown\n", token);
	exit(EXIT_FAILURE);
	}
}
